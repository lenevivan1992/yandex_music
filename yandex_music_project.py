...
# Возможно, следует переименовать 'userID' в 'user_id'. ("Змеиный регистр")
...
sorted_genre_df = df.sort_values(by='genre') 
print(sorted_genre_df['genre'].unique()) # Просмотр уникальных названий жанров
...
#for true_hiphop in duplicates_hiphop:
    #df['genre'] = df['genre'].replace('true_hiphop', 'hiphop') думаю, так тоже прокатило бы...
...
sorted_genre_df = df.sort_values(by='genre') 
print(sorted_genre_df['genre'].unique()) # Проверка на неявные дубликаты
...
columns_number_tracks = ['city', 'monday', 'wednesday', 'friday']
data_number_tracks = [['Moscow', 15740, 11056, 15945], ['Saint-Petersburg', 5614, 7003, 5895]]
dataframe_number_tracks = pd.DataFrame(data=data_number_tracks, columns=columns_number_tracks)
print(dataframe_number_tracks)   # Таблица с результатами
